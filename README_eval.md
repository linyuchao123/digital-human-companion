...
该模式的样本数和候选数不满足官方协议，输出只用于验证模型能否完成真实推理，
不能用于计算或对外展示正式指标。旧脚本遇到空标签时会静默补零；当前实现会直接停止，
以免产生失真的评测结果。

完整生成后，在计算指标前检查最终文件：

```bash
.venv-model/bin/python scripts/check_avatar_prediction.py
```

该检查通过内存映射逐条扫描，不会把约 800 MB 的预测数组完整复制到内存；只有形状为
`[1086, 10, 750, 25]`、类型为 `float32` 且不包含 `NaN` 或无穷值时才会通过。

...
![数字心屿连接数字人页面](docs/images/readme/connection-screen.png)

新账号第一次进入时会显示居中的使用指南；关闭后状态保存到账号，后续登录不再自动弹出，也可以从顶部“使用指南与反馈”随时重新打开。

![数字心屿首次使用指南](docs/images/readme/first-use-guide.png)

### 日常对话与情感对话

两种模式共享同一个数字人和连续会话空间，但采用不同的回应策略：日常对话适合轻松交流、问候与生活分享；情感对话更重视倾听、复述和循序渐进的情绪梳理。

| 日常对话模式 | 情感对话模式 |
| --- | --- |
| ![数字心屿日常对话模式](docs/images/readme/daily-dialogue.png) | ![数字心屿情感对话模式](docs/images/readme/emotional-dialogue.png) |

### 心理学知识库

管理员可以浏览、录入并检索带来源信息的心理教育知识；普通用户通过对话中的 RAG 链路获得知识辅助回答，不会看到管理入口。

![数字心屿心理学知识库](docs/images/readme/rag-knowledge-base.png)

### 移动端已适配

项目已经为 iOS、Android、微信内置浏览器及常见移动 WebView 提供响应式布局：顶部功能入口保持可用，历史会话使用独立抽屉，对话区与数字人分区显示，支持“按住说话、松开发送”，摄像头预览缩为右上角小窗。登录与注册页采用单层触摸滚动，并适配刘海屏、底部安全区和软键盘场景，长表单可以完整滚动到提交区域。

<p align="center">
  <img src="docs/images/readme/mobile-workspace.jpg" width="360" alt="数字心屿移动端适配界面">
</p>
